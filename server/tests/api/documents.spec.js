...
describe('Document API', () => {
  ...
  describe('GET: /documents', () => {
    ...
    describe('should return documents from the most recent', () => {
      before((done) => {
        db.Document.bulkCreate(DocumentHelper.document1,
          DocumentHelper.document2);
        request.post('/api/documents')
      ...
      it(
        'ensures that a user can search authorised documents with keywords',
        (done) => {
          request.get('/api/documents/search?term=a')
            .set({ Authorization: token })
            .expect(200)
            .end((err, res) => {
              expect((res.body.doc).length).to.be.greaterThan(
                0);
      ...
      it(
        'ensures that a user can search authorised documents with keywords',
        (done) => {
          request.get('/api/documents/search?term=o')
            .set({ Authorization: token })
            .expect(200)
            .end((err, res) => {
              expect((res.body.doc).length).to.be.greaterThan(
    ...
    it('returns 1 for `page` when a limit is not set', (done) => {
      request.get('/api/documents')
        .set({ Authorization: token })
        .end((err, response) => {
    ...
    it('return `0` for page_size when offset is not set', (done) => {
      request.get('/api/documents')
        .set({ Authorization: token })
        .end((err, response) => {
          expect(response.body.pagination.page_size).to.equal(10);
          done();
        });
    });
